...
function calcTotal() {
   ...
   if (isNaN(wgtBox.value) || wgtBox.value <= 0) {
      msgBox.innerHTML = "Please enter a valid weight greater than 0.";
      return; 
   } else {
      totalCost += wgtBox.value * COST_PER_LB;      
   }
   
   if (isNaN(distBox.value) || distBox.value <= 0) {
      msgBox.innerHTML = "Please enter a valid distance greater than 0.";
      return;
   } else {
